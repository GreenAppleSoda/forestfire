"""
기상청 API 허브 — 종관기상관측(ASOS) 시간/일 자료 조회·파싱 (ml-service 전용 사본)

etl/kma_asos_client.py 와 로직은 같지만, etl/ 폴더 없이도 동작하도록
인증키를 os.environ(ml-service/.env가 이미 로드해 둔 값)에서만 읽습니다.
